import 'package:flutter/material.dart';

class ColorsManager{
  static const Color mainPurple = Color(0xFF6739FF);
  static const Color lightPurple = Color(0xFF997AFF);
  static const Color shadowPurple = Color(0xFF6739FF);
  static const Color mainWhite = Color(0xFFF0EBFF);
  static const Color lightWhite = Color(0xFFD1D1D1);
  static const Color lighterWhite = Color(0xFFBFBFBF);
  static const Color darkGray = Color(0xFF151515);
  static const Color lightGray = Color(0xFF232323);
  static const Color lighterGray = Color(0xFF313131);



}